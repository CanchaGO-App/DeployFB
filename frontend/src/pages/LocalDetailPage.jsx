          ...
          <div style={{ display: 'flex', gap: '32px', marginBottom: '48px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
            <div style={{ position: 'relative', width: '350px', height: '250px', borderRadius: 'var(--radius-lg)', overflow: 'hidden', flexShrink: 0, boxShadow: 'var(--shadow-card)' }}>
              {getLocalImg(local) ? (
                ...
                <img src={getLocalImg(local)} alt={local.nombre} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />  // HARDCODED
                ...
                {local.canchas.map((cancha) => {
                  ...
                  return (
                    <div key={cancha.id} style={{ 
                      ...
                      {getCanchaImg(cancha) ? (
                        // HARDCODED — ANTES: cancha.imagen_url ? (
                        <img src={getCanchaImg(cancha)} alt={cancha.nombre} style={{ width: '100%', height: '160px', objectFit: 'cover', borderRadius: 'var(--radius-md)', marginBottom: '16px' }} />  // HARDCODED
                        // ANTES: src={cancha.imagen_url}
                      ) : (
